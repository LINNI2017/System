...
void TestRemoveCardFromHand() {
  ...
  assert(hand.first_card->this_card == &card2);
  assert(hand.first_card->next_card == NULL);
  // PrintCard(hand.first_card->this_card);
  // PrintCard(hand.first_card->prev_card->this_card);
  assert(hand.first_card->prev_card == NULL);
...
void TestReturnHandToDeck() {
  StartTest("return hand to deck");
  // TODO(ahs): Make this test more robust
  Deck* deck = PopulateDeck();
  Shuffle(deck);
  // PrintDeck(deck);
  ...
  // return cards of hand1 to deck
  ReturnHandToDeck(&hand1, deck);
  // PrintDeck(deck);
  int idx = deck->top_card;
  for (int i = 0; i < kNumCardsInHand; i++) {
    assert(deck->cards[idx--] == expected_hand1[i]);
  }

  // return cards of hand2 to deck
  ReturnHandToDeck(&hand2, deck);
  idx = deck->top_card;
  for (int i = 0; i < kNumCardsInHand; i++) {
    assert(deck->cards[idx--] == expected_hand2[i]);
  }
